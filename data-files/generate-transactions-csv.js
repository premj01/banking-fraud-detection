// Script to generate 100 transaction records CSV
...
function generateTransaction(profile, index, isFraud = false) {
    ...
    if (isFraud) {
        ...
        if (fraudType < 0.33) {
            ...
            // Geographic fraud - pick a far city
    ...
    return {
        ...
        receiver_bank: ['HDFC Bank', 'ICICI Bank', 'State Bank of India', 'Axis Bank', 'Kotak Mahindra Bank'][Math.floor(Math.random() * 5)],
...
async function generateCSV() {
    ...
    if (profiles.length === 0) {
        console.error('❌ No client profiles found! Run seed-client-profiles.js first.')
        return
    }
    ...
    console.log(`✅ Generated 100 transactions (85 normal, 15 fraud)`)
    console.log(`📁 Saved to: ${outputPath}`)
    console.log('\nSample transactions:')
    transactions.slice(0, 3).forEach(t => {
        console.log(`  - ${t.transaction_id}: ${t.sender_customer_id}, Amount: ${t.amount_value}`)
    })
}
