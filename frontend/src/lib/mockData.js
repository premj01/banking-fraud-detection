// Helper functions
const randomId = (prefix) => `${prefix}${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
...
export const generateTransaction = (overrides = {}) => {
    ...
    const deviceType = randomElement(['mobile', 'web', 'tablet']);
    ...
    return {
        ...
        currency: 'INR',
        transaction_type: randomElement(['transfer', 'card', 'withdrawal', 'online_purchase']),
...
export const generateFraudAlert = (overrides = {}) => {
    const alertType = randomElement(alertTypes);
    const severity = randomElement(['Critical', 'High', 'Medium', 'Low']);
    const status = randomElement(['Open', 'Under Review', 'Closed', 'Escalated']);
...
export const generateInvestigator = () => {
    const firstName = randomElement(firstNames);
    const lastName = randomElement(lastNames);
    ...
    return {
        ...
        role: randomElement(['Fraud Analyst', 'Investigator']),
...
export const generateGeoHeatmapData = () =>
    locations.map(loc => ({
        city: loc.city,
        state: loc.state,
        country: loc.country,
...
export const generateModelPerformance = () => {
    const data = [];
    for (let i = 29; i >= 0; i--) {
        const date = new Date();
...
export const generateFraudAlerts = (count) =>
    Array.from({ length: count }, () => generateFraudAlert());

export const generateInvestigators = (count) =>
...
export const generateHourlyData = (baseValue, variance) => {
    const data = [];
    const now = new Date();
    for (let i = 23; i >= 0; i--) {
